...
import { Plus, Search, Pencil, Trash2, X, AlertCircle, Loader2, ChevronLeft, ChevronRight, UserPlus, Mail, Phone, CalendarDays, MapPin, Copy, Check } from 'lucide-react'
...
export default function StudentsPage() {
  ...
  const { data, isLoading, error } = useStudents({ page, pageSize: 20, search, classId: classFilter || undefined })
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 lg:mb-6">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold text-text-primary">Élèves</h1>
          <p className="text-sm text-text-muted mt-1">
            {meta.total} élève{meta.total > 1 ? 's' : ''} · Page {meta.page}/{Math.ceil(meta.total / meta.pageSize) || 1}
          </p>
        </div>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-card border border-border rounded-xl text-sm text-text-primary placeholder:text-text-muted/60 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all duration-200"
          />
        </div>
        <select
          value={classFilter}
          onChange={(e) => { setClassFilter(e.target.value); setPage(1) }}
          className="px-3.5 py-2.5 bg-card border border-border rounded-xl text-sm text-text-primary focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
        >
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-text-muted text-sm">{search || classFilter ? 'Aucun résultat' : 'Aucun élève pour le moment'}</p>
          {!search && !classFilter && (
            <button onClick={openCreate} className="mt-4 text-primary text-sm font-medium hover:underline">Ajouter un élève</button>
          )}
        </div>
      ) : (
        <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="text-left text-xs font-semibold text-text-muted uppercase tracking-wider px-6 py-4">Genre</th>
                    <th className="text-left text-xs font-semibold text-text-muted uppercase tracking-wider px-6 py-4">Parent</th>
                    <th className="text-left text-xs font-semibold text-text-muted uppercase tracking-wider px-6 py-4">Année</th>
                  ...
                  {students.map((s, i) => (
                    ...
                    >
                      <td className="px-6 py-4">
                        <span className="text-sm font-mono font-semibold text-primary">{s.studentCode}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-medium text-text-primary">{s.lastName} {s.firstName}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm text-text-muted">{getClassName(s.classId)}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`text-sm font-medium ${s.gender === 'M' ? 'text-primary' : 'text-purple'}`}>
                          {s.gender === 'M' ? 'Masculin' : 'Féminin'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-text-muted">
                          {s.parentName && <p>{s.parentName}</p>}
                          {s.parentPhone && <p className="text-xs">{s.parentPhone}</p>}
                          {!s.parentName && !s.parentPhone && <span>—</span>}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm text-text-muted">{s.schoolYear || '—'}</span>
                      </td>
                      <td className="px-6 py-4">
            ...
            {meta.total > meta.pageSize && (
              <div className="flex items-center justify-between px-4 lg:px-6 py-3 lg:py-4 border-t border-border">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setPage((p) => p + 1)} disabled={meta.page >= Math.ceil(meta.total / meta.pageSize)}
                    className="p-2 rounded-lg text-text-muted hover:text-primary hover:bg-primary/5 transition-all disabled:opacity-30 disabled:cursor-not-allowed">
      ...
      <AnimatePresence>
        {modal === 'form' && (
          <StudentFormModal
            ...
            classes={classes}
            onCreated={(student) => setCreatedStudent(student)}
          />
        )}
        {deleteId && (
          <DeleteConfirmModal
...
function StudentFormModal({ editId, onClose, createMutation, updateMutation, classes, onCreated }) {
  const { data: studentData, isLoading: loadingStudent } = useStudent(editId)
  const [form, setForm] = useState({
    firstName: '', lastName: '', birthDate: '', birthPlace: '',
    gender: 'M', classId: '', schoolYear: '2025-2026',
    parentName: '', parentPhone: '', parentEmail: '',
    hasSmartphone: false, isFirstYear: true,
  })
  ...
  useEffect(() => {
    if (studentData) {
      setForm({
        ...
        hasSmartphone: studentData.hasSmartphone || false,
        isFirstYear: studentData.isFirstYear !== false,
      })
    }
  }, [studentData])
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.classId) errs.classId = 'Classe requise'
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-card border border-border rounded-2xl lg:rounded-3xl w-full max-w-xl p-4 lg:p-6 shadow-premium-lg my-8"
      >
        {loadingStudent && editId ? (
          ...
          <>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-lg font-bold text-text-primary">{editId ? 'Modifier' : 'Ajouter'} un élève</h2>
                <p className="text-sm text-text-muted mt-0.5">
                  {editId ? 'Modifier les informations' : 'Nouvel élève dans le système'}
                </p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="block text-sm font-medium text-text-primary">Date de naissance</label>
                  <input name="birthDate" type="date" value={form.birthDate} onChange={handleChange}
                    className={`w-full px-3.5 py-2.5 text-sm bg-bg border rounded-xl text-text-primary focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all ${errors.birthDate ? 'border-danger' : 'border-border'}`} />
                  {errors.birthDate && <p className="text-xs text-danger">{errors.birthDate}</p>}
                </div>
                <div className="space-y-1.5">
                  <label className="block text-sm font-medium text-text-primary">Lieu de naissance</label>
                  <input name="birthPlace" value={form.birthPlace} onChange={handleChange} placeholder="Yaoundé"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="block text-sm font-medium text-text-primary">Classe</label>
                  <select name="classId" value={form.classId} onChange={handleChange}
                    className={`w-full px-3.5 py-2.5 text-sm bg-bg border rounded-xl text-text-primary focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all ${errors.classId ? 'border-danger' : 'border-border'}`}>
                    <option value="">Sélectionner...</option>
                    {classes.map((c) => (
                      <option key={c.id} value={c.id}>{c.name}</option>
                    ))}
                  </select>
                  {errors.classId && <p className="text-xs text-danger">{errors.classId}</p>}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="block text-sm font-medium text-text-primary">Email parent</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="block text-sm font-medium text-text-primary">Parent / Tuteur</label>
                  <input name="parentName" value={form.parentName} onChange={handleChange} placeholder="Nom du parent"
                    className="w-full px-3.5 py-2.5 text-sm bg-bg border border-border rounded-xl text-text-primary placeholder:text-text-muted/60 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all" />
                </div>
                <div className="space-y-1.5">
                  <label className="block text-sm font-medium text-text-primary">Téléphone parent</label>
              ...
              <div className="flex items-center gap-6 pt-2">
                <label className="flex items-center gap-2 cursor-pointer group">
                  <input type="checkbox" name="hasSmartphone" checked={form.hasSmartphone} onChange={handleChange}
                    className="w-4 h-4 rounded border-border text-primary focus:ring-primary/30 cursor-pointer" />
                  <span className="text-sm text-text-muted group-hover:text-text-primary transition-colors">Parent a un smartphone</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer group">
                  <input type="checkbox" name="isFirstYear" checked={form.isFirstYear} onChange={handleChange}
                    className="w-4 h-4 rounded border-border text-primary focus:ring-primary/30 cursor-pointer" />
                  <span className="text-sm text-text-muted group-hover:text-text-primary transition-colors">Première année</span>
                </label>
              </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 className="text-lg font-bold text-text-primary mb-2">Supprimer cet élève ?</h2>
        <p className="text-sm text-text-muted mb-6">Cette action est irréversible. L'élève sera désactivé du système.</p>
...
function StudentCreatedModal({ student, onClose }) {
  return (
    <motion.div
      initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 z-50 flex items-start sm:items-center justify-center overflow-y-auto p-4 bg-black/40 backdrop-blur-sm"
    ...
    >
      ...
      >
        ...
        <h2 className="text-lg font-bold text-text-primary mb-1">Élève créé</h2>
        <p className="text-sm text-text-muted mb-5">
          Code : <span className="font-mono font-semibold text-primary">{student.studentCode}</span>
        </p>
        <div className="bg-bg border border-border rounded-xl p-4 mb-5 text-left text-sm text-text-muted">
          <p>L'élève <strong className="text-text-primary">{student.firstName} {student.lastName}</strong> a été enregistré avec succès.</p>
        </div>
